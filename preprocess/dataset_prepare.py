...
def get_neighbor_distance(mask, distance_func=distance_transform_edt):
    ...
    for shape_id in np.unique(mask):
        ...
        invert_dt = distance_transform_edt(
...
def create_dataset(results, path, distance_func=distance_transform_edt):
    dataset_file_init(path)
    CPU_NUM = multiprocessing.cpu_count()
    func = partial(split_image, distance_func=distance_func)
    size = 10
...
if __name__ == "__main__":
    ...
    create_dataset(results, path=dataset_path, distance_func=poisson_equation)
